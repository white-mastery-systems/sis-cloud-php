...
/***** PRODUCTS *****/
    ...
    //designation list
      public function getuserdesignation($getData) 
      {
          $response = array();
          $jsonData = json_decode($getData, true);
          
           //department
           $response["department_list"] = $this->internaldepartmentlist();

          if($jsonData['cancel_status']!='none')
          {
              // collection
              $collection = $this->conn->user_designation;
              $response['user_designation'] = array();
  
              if($jsonData['cancel_status'] == '1'){ $cancelStatus = "Active"; }
              else{ $cancelStatus = "Cancel"; }
              ...
              if($result)
              {
                  foreach($result as $rowData)
                  {
                      ...
                      array_push($response["user_designation"], $sendData);
                      }
                  }
                  $response["code"] = "Success";
              }
     ...
     //adddesignation
 public function adddesignation($getData) 
 {
     $response = array();
   
     $collectionDesignation = $this->conn->user_designation;
     $jsonData = json_decode($getData, true);        
     $designationData = $collectionDesignation->findOne(array('$or' => array(        
         array(
             "department" =>  $jsonData["department"],
             "designation_ws" => $jsonData["designation_ws"]
             )
     )));
     if(!$designationData) {     
         $collectionDesignation->insertOne(array(
             "emp_id" => (int)$jsonData["emp_id"],
             "department" => $jsonData["department"],  
         ...
         $response["code"] = "Designation details already exist !!!";
     }
     return $response;
 }

  //edit department
  public function editdesignation($getData) 
  {
      $response = array();
       //department
       $response["department_list"] = $this->internaldepartmentlist();
      $collectionDesignation = $this->conn->user_designation;
      $jsonData = json_decode($getData, true);      
      $designationData = $collectionDesignation->findOne(array("_id" => new MongoDB\BSON\ObjectID($jsonData['id'])));
      if($designationData) {
          $designationData['department'] = $designationData['department'];
          $designationData['designation'] = $designationData['designation'];
          $designationData['id'] = $designationData['_id'];
          $response["designation_details"] = $designationData;
      }
      return $response;
  }
...
//update department
public function updatedesignation($getData) 
{
    $response = array();
    $collection = $this->conn->user_designation;    
    $jsonData = json_decode($getData, true);
    $count = $collection->count(array(      
        ...
        "department" =>  $jsonData["department"],
        "designation" =>  $jsonData["designation_ws"]            
    ));
...
public function deletedesignation($getData)
{
    $response = array();
    ...
    else
    {
        ...
        $cursor = $collection->updateOne(
            array("_id" => new MongoDB\BSON\ObjectID($id)),
            array('$set' => array("status" => $cancelStatus))
        );
    }
...
//department list
public function internaldepartmentlist() 
{
    $response = array();
    $collection = $this->conn->user_department;  
    ...
    if($result)
    {
        ...
        foreach($result as $rowData)
        {
            $department['status'] = $rowData['status'];
            $department['department'] = $rowData['department'];
    ...
    //department list
      public function getuserdepartment($getData) 
      {
          $response = array();
          $jsonData = json_decode($getData, true);
          ...
          if($jsonData['cancel_status']!='none')
          {
              // collection
              $collection = $this->conn->user_department;
              $response['user_department'] = array();
            ...
            //   else{ $cancelStatus = 1; }
              ...
              if($result)
              {
                  foreach($result as $rowData)
                  {
                      ...
                      {                     
                      $sendData['_id'] = $rowData['_id'];
                      $sendData['name'] = $rowData['department'];
                      $sendData['status'] = $rowData['status'];
                      $sendData['emp_id'] = $rowData['emp_id'];                     
                      array_push($response["user_department"], $sendData);
                      }
                  }
                  $response["code"] = "Success";
              }
 ...
 //adddepartment
 public function adddepartment($getData) 
 {
     $response = array();
     $collectionDepartment = $this->conn->user_department;
     $jsonData = json_decode($getData, true);        
     $departmentData = $collectionDepartment->findOne(array('$or' => array(        
         array("department_ws" =>  $jsonData["name_ws"])
     )));
     if(!$departmentData) {     
         $collectionDepartment->insertOne(array(
             "emp_id" => (int)$jsonData["emp_id"],
             "department" => $jsonData["name"],  
             "department_ws" => $jsonData["name_ws"], 
             "status" =>'1',          
             "date_time" => date("Y-m-d H:i:s")
         ));
         $response["code"] = "Success";
         ...
         $response["code"] = "Department details already exist !!!";
     }
     return $response;
 }

  //edit department
  public function editdepartment($getData) 
  {
      $response = array();
      $collectionDepartment = $this->conn->user_department;
      $jsonData = json_decode($getData, true);      
      $departmentData = $collectionDepartment->findOne(array("_id" => new MongoDB\BSON\ObjectID($jsonData['id'])));
      if($departmentData) {
          $departmentData['department'] = $departmentData['department'];
          $departmentData['id'] = $departmentData['_id'];
          $response["department_details"] = $departmentData;
      }
      return $response;
  }
...
//update department
public function updatedepartment($getData) 
{
    $response = array();
    $collection = $this->conn->user_department;    
    $jsonData = json_decode($getData, true);
    $count = $collection->count(array(      
        "status" => $jsonData["status"],
        "department_ws" =>  $jsonData["department_ws"]        
    ));
    $response["count"] = $count;
...
public function deletedepartment($getData)
{
    $response = array();
...
public function getprocategory($getData) 
{
    $response = array();
    $jsonData = json_decode($getData, true);
      ...
      //   else{ $cancelStatus = 1; }
        ...
        if($result)
        {
            ...
            {                
                ...
                $sendData['_id'] = $rowData['_id'];
                $sendData['category'] = $rowData['category'];
                $sendData['short'] = $rowData['short'];
                $sendData['type'] = $rowData['type'];
                $sendData['emp_id'] = $rowData['emp_id'];                     
                array_push($response["category"], $sendData);
                }
            }
            $response["code"] = "Success";
        }
...
//edit category 
public function editcategory($getData) 
{
    $response = array();
    $collectionCategory = $this->conn->category;
    $jsonData = json_decode($getData, true);      
    $categoryData = $collectionCategory->findOne(array("_id" => new MongoDB\BSON\ObjectID($jsonData['id'])));
    if($categoryData) {
        $categoryData['category'] = $categoryData['category'];
        $categoryData['id'] = $categoryData['_id'];
        $categoryData['short'] = $categoryData["short"];
        $categoryData["category_ws"] = $categoryData["category_ws"];
        $response["category_details"] = $categoryData;
    }
    return $response;
}
...
//update category
public function updatecategory($getData) 
{
  $response = array();
  $collection = $this->conn->category;    
  $jsonData = json_decode($getData, true);
...
public function deletecategory($getData)
{
  $response = array();
...
public function getprotype($getData) 
{
    $response = array();
    $jsonData = json_decode($getData, true);
    if($jsonData['cancel_status']!='none')
    {
        // collection
        $collection = $this->conn->protype;
        $response['protype_details'] = array();
        ...
        if($result)
        {
            ...
            {                
                ...
                $sendData['status'] = $rowData['status'];
                $sendData['emp_id'] = $rowData['emp_id'];                     
                array_push($response["protype_details"], $sendData);
                }
            }
            $response["code"] = "Success";
        }
    ...
    //add product type
    public function addprotype($getData) 
    {
        $response = array();
        $collectionType = $this->conn->protype;
        $jsonData = json_decode($getData, true);        
        $typeData = $collectionType->findOne(array('$or' => array(
            array("type_ws" => $jsonData["type_ws"])
           
        )));
        if(!$typeData) {
         $type = trim($jsonData["type"]);
            $collectionType->insertOne(array(
                "emp_id" => (int)$jsonData["emp_id"],
                "type" => $type,    
                "type_ws" => $jsonData["type_ws"],            
                "status" => $jsonData["status"],               
                "date_time" => date("Y-m-d H:i:s")
            ));
            $response["code"] = "Success";
            ...
            $response["code"] = "Product Type already exist !!!";
        }
        return $response;
    }
...
//edit type
public function editprotype($getData) 
{
    $response = array();
    $collectionType = $this->conn->protype;
    $jsonData = json_decode($getData, true);      
    $TypeData = $collectionType->findOne(array("_id" => new MongoDB\BSON\ObjectID($jsonData['id'])));
    if($TypeData) {
        $TypeData['type'] = $TypeData['type'];
        $TypeData['id'] = $TypeData['_id'];
        $TypeData['status'] = $TypeData["status"];
        $TypeData["emp_id"] = $TypeData["emp_id"];
        $response["type_details"] = $TypeData;
    }
    return $response;
}
...
//update department
public function updateprotype($getData) 
{
  $response = array();
  $collection = $this->conn->protype;    
  $jsonData = json_decode($getData, true);
...
public function deleteprotype($getData)
{
  $response = array();
...
public function getmailsettings($getData) 
{
    ...
    {
        // collection
        if($jsonData['mailtype'] == 'to' )
        {
            $collection = $this->conn->mail_to;
        }
        else if($jsonData['mailtype'] == 'cc')
        {
            $collection = $this->conn->mail_cc;
        }
        else if($jsonData['mailtype'] == 'bcc')
        {
            $collection = $this->conn->mail_bcc;
        }
        
        $response['mail_details'] = array();
        ...
        if($result)
        {
            ...
            {                
                ...
                {                     
                $sendData['_id'] = $rowData['_id'];
                $sendData['mail_id'] = $rowData['mail_id'];
                $sendData['type'] = $rowData['type'];
                $sendData['mailtype']  = $jsonData['mailtype'];
                $sendData['status'] = $rowData['status'];
                $sendData['emp_id'] = $rowData['emp_id'];                     
                array_push($response["mail_details"], $sendData);
                }
            }
            $response["code"] = "Success";
        }
    ...
    public function addmailsettings($getData) 
    {
        $response = array();
        $jsonData = json_decode($getData, true);
        ...
        //emp id
        $cursor = $collectionuser->findOne(array(            
            "mail_id" => $mail_id,
            "status" => 'Active'
        ));
        if($cursor)
        {
            ...
            $mail_id = trim($jsonData["mail_id"]);       
            $mailData = $collection->findOne(array(
                "type" => $jsonData["type"], 
                "mail_id" => $mail_id
            ));
            if(!$mailData) {
             $mail_id = trim($jsonData["mail_id"]);
                ...
                $response["code"] = "Mail ID already exist !!!";
            }  
           
        }
        else
        {
            $response["code"] = "Mail ID is not a Active User !!!";
        }
...
//edit type
public function editmailsettings($getData) 
{
    ...
    $mailData = $collection->findOne(array("_id" => new MongoDB\BSON\ObjectID($jsonData['id'])));
    if($mailData) {
        $mailData['id'] = $mailData['_id'];
        $mailData['mail_id'] = $mailData["mail_id"];     
        $mailData['mailtype'] = $mailData["mailtype"]; 
        $mailData['type'] = $mailData["type"];  
        $mailData['status'] = $mailData["status"]; 
        $response["mail_details"] = $mailData;
    }
    return $response;
...
//update department
public function updatemailsettings($getData) 
{
  $response = array();
  $jsonData = json_decode($getData, true);
...
public function deletemailsettings($getData)
{
  $response = array();
  $jsonData = json_decode($getData, true);
  // collection
...
public function getmailsmtpsettings($getData) 
{    
    $response = array();
    $jsonData = json_decode($getData, true);
    if($jsonData['cancel_status']!='none')
    {
        // collection
      $collection = $this->conn->mailsettings;
      $response['mail_details'] = array();
        ...
        if($result)
        {
            ...
            {                
                ...
                {                     
                $sendData['_id'] = $rowData['_id'];
                $sendData['host'] = $rowData['host'];
                $sendData['type'] = $rowData['type'];
                $sendData['port'] = $rowData['port'];
                $sendData['secure'] = $rowData['secure'];
                $sendData['status'] = $rowData['status'];
                $sendData['emp_id'] = $rowData['emp_id'];                     
                array_push($response["mail_details"], $sendData);
                }
...
//add product type
public function addmailsmtpsettings($getData) 
{
    $response = array();
    $jsonData = json_decode($getData, true);
    $collection = $this->conn->mailsettings;
    ...
    if(!$mailData) {
        ...
        $response["code"] = "Host already exist !!!";
    }
    return $response;
}
...
//edit type
public function editmailsmtpsettings($getData) 
{
    ...
    if($mailData) {
        $mailData['id'] = $mailData['_id'];
        $mailData['host'] = $mailData["host"];     
        $mailData['port'] = $mailData["port"]; 
        $mailData['type'] = $mailData["type"]; 
        $mailData['secure'] = $mailData["secure"]; 
        $mailData['status'] = $mailData["status"]; 
        $response["smtp_details"] = $mailData;
    }
    return $response; 
}
...
//update department
public function updatemailsmtpsettings($getData) 
{
  $response = array();
  $jsonData = json_decode($getData, true);
  $collection = $this->conn->mailsettings;
...
public function deletemailsmtpsettings($getData)
{
  $response = array();
  $jsonData = json_decode($getData, true);
  $collection = $this->conn->mailsettings;
...
/***** INDENT *****/
    ...
    public function getindentlist($getData)
    {
        ...
        if($result)
        {
            ...
            {
		$datetime = new DateTime($rows['prf_date']);
                if(!$rows['quot_status']){ $rows['quot_status']=''; }
                $product['prf_number'] = $rows['prf_number'];
                $product['prf_date'] =  $datetime->format('d M Y'); 
    ...
    public function getindentdetails($getData)
    {
        ...
        $response["pro_type"] = $this->internalprotype('1');
        //category
        $response["category_list"] = $this->internalcategorylist();
    ...
    public function viewpurchaseindent($getData) 
    {
		...
		$datetime = new DateTime($rows['prf_date']);
                ...
                $sendData['prf_date'] = $datetime->format('d M Y'); 
		   ...
		   $reqdatetime = new DateTime($rowData['indent_table_data'][$i]['required_date']);
                    ...
                    $purchaseData['required_date']= $reqdatetime->format('d M Y');
    ...
    public function getindentmailcontent($getData) 
    {
        ...
        $collectionMaster = $this->conn->indent_master;
        $collectionUser = $this->conn->signintable;
        $response["mail_to"] = $this->internalmailsetting('to','indent');
        $response["mail_cc"] = $this->internalmailsetting('cc','indent');
        $response["mail_bcc"] = $this->internalmailsetting('bcc','indent');
        ...
        if($blockName != 'none') {
            $block = " - ".$blockName."Department";
        }
        $response['content'] = "Dear Sir/Ma'am,<br><br>Please find the attached Indent form, for items needed for <b>".$projectName.$block.".</b>";
...
public function internalmailsetting($mailtype,$type) 
{
    $response = array();
    ...
    if($result)
    {
        ...
        foreach($result as $rowData)
        {
            $mailarr['mail_id'] = $rowData['mail_id'];           
            array_push($response, $mailarr);
        }
        return $response;
    }    
}

public function internalprotype($status) 
{
    $response = array();
    ...
    if($result)
    {
        ...
        foreach($result as $rowData)
        {
            $typearr['type'] = $rowData['type'];           
            array_push($response, $typearr);
        }
        return $response;
    }    
}
    ...
    public function sendindentmail($getData)
    {
        ...
        {
            //mail
            file_get_contents('https://cloud.sis.in/old/modules/materials/indent/indent_pdf.php?type=upload&id='.$jsonArrayData['prf_number']);
            sleep(8);
	//$data = file_exists('https://cloud.sis.in/old/modules/materials/indent/'.$jsonArrayData['prf_number'].'.pdf')
           $data = $_SERVER['DOCUMENT_ROOT'].'/old/modules/materials/indent/'.$jsonArrayData['prf_number'].'.pdf'));
            if(file_exists($_SERVER['DOCUMENT_ROOT'].'/old/modules/materials/indent/'.$jsonArrayData['prf_number'].'.pdf'))
            {
                $indentUrl = $this->path.'/old/#/indent_mail/'.$jsonArrayData["prf_number"];                
...
/***** QUOTATION *****/  
    ...
    public function getquotationmailcontent($getData) 
    {
        ...
        $collectionMaster = $this->conn->quotation_master;
        $collectionUser = $this->conn->signintable;
        $response["mail_to"] = $this->internalmailsetting('to','quotation');
        $response["mail_cc"] = $this->internalmailsetting('cc','quotation');
        $response["mail_bcc"] = $this->internalmailsetting('bcc','quotation');
        ...
        foreach($cursorData as $indentData)
        {
            if($indentData["project_data"][0]["block_name"] != 'none') {
                $block = " - ".$indentData["project_data"][0]["block_name"]." Department";
            }
            if(!$jsonData['purpose']) {
                $jsonData['purpose'] = $indentData['purpose'];
            }
            $response['content'] = "Dear Sir/Ma'am,<br><br>Please find the attached Quotation form, for items which are needed for our <b>".$indentData["project_data"][0]['project_name'].$block.".</b>";
    ...
    public function sendquotationmail($getData)
    {
        ...
        if($cursor)
        {
            ...
            {
                //mail
                file_get_contents($this->path.'/old/modules/materials/quotation/quotation_pdf.php?type=upload&id='.$jsonArrayData['quot_number']);
                sleep(8);

                if(file_exists($_SERVER['DOCUMENT_ROOT'].'/old/modules/uploads/'.$jsonArrayData['quot_number'].'.pdf'))
                {
                    $tableContent = "<br><br>
                    ...
                    $approveUrl = $this->path.'/old/#/quotation_approve/'.$jsonArrayData["quot_number"];
    ...
    public function approvequotation($getData)
    {
        ...
        //mail
        file_get_contents($this->path.'/old/modules/materials/quotation/quotation_pdf.php?type=upload&id='.$jsonData['quot_number']);
        sleep(8);
    
        if(file_exists($_SERVER['DOCUMENT_ROOT'].'/old/modules/uploads/'.$jsonData['quot_number'].'.pdf'))
        {
...
/***** PURCHASE ORDER *****/
    ...
    public function viewpurchaseorder($getData) 
    {
        ...
        if($cursor)
        {
            foreach($cursor as $rowData)
            {
				...
				$sendData['plain_amount'] = $rowData['trans_amount'];
                $sendData['plain_tax'] = $rowData['trans_tax'];
    ...
    public function sendpomail($getData)
    {
        ...
        file_get_contents($this->path.'/old/modules/materials/purchase/po_pdf.php?type=upload&id='.$jsonData['po_number'].'&emp_id='.$jsonData['emp_id']);
		sleep(8);
            
       if(file_exists($_SERVER['DOCUMENT_ROOT'].'/old/modules/materials/purchase/'.$jsonData['po_number'].'.pdf'))
        {
        ...
        if($cursor) 
        {
            ...
            foreach($cursor as $rowData){
				...
				$sendData['vendorname'] = $rowData['purchase_data'][0]['company'];   
				 ...
				 $vendorData = $collectionVendor->findOne(array("company_name" => $rowData['purchase_data'][0]['company']));
				if($vendorData)
				{
					$vendor_mobile = $vendorData['mobile'];
				}
				
				 //$sendData1 = json_decode($sendData, true);
//                 $vendorData = $collectionVendor->findOne(array('company_name' => $rowData['purchase_data'][0]['company']));
//				if($vendorData) {
//				   $sendData['vendor_mobile'] = $vendorData['mobile']);
//				}
//        
...
/***** INVENTORY USE *****/
    ...
    public function getinventoryuselist($getData)
    {
        ...
        $cursor = $collection->aggregate(array(
            ...
            array( '$group' => array(
                ...
                'inventory_id' => array('$push' => array('_id' => '$_id')),
                'stock' => array('$sum' => '$stock'),
                'usage' => array('$sum' => '$usage'),
                'breakage' => array('$sum' => '$breakage'),
                'transfer' => array('$sum' => '$transfer'),
                'available' => array('$sum' => '$available'),
        ...
        if ($cursor) {
            ...
            foreach($cursor as $rowData){
                ...
                $sendData['project_name'] = $projectData['project_name'];
                $sendData['block_name'] = $projectData['block_name'];
    ...
    public function stockupdate_use($getData)
    {
		...
		for($i=0; $i<$count; $i++)
        {
            $id = $jsonArrayData['inventory_id'][$i]['_id']['$oid'];
...
/***** USER LIST *****/
    ...
    public function adduserdetails()
    {
        $response=array();
        $collection = $this->conn->user_list;
        $collectionDprt = $this->conn->user_designation;
        $collectionProject = $this->conn->project_details;
