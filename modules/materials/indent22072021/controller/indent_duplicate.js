app.controller('indentDuplicateCtrl', function($scope, $http, dateFilter, $state, $stateParams, $timeout, orderService) {
    ...
    else
    {
        ...
        $http({
            ...
            method: "POST",
            data: {'type': 'duplicate', 'prf_number': prfNum, 'emp_id': localStorage.getItem("user_id")}
        })
        .then(function (response){
            ...
            $scope.indentForm.prf_date = $scope.poDate;
		...
		return (mail_id._lowername.indexOf(lowercaseQuery) === 0);
		 ...
		 $scope.mailContent = function(){			
			...
			$scope.cc_mail = [];
			$scope.bcc_mail = [];
			$scope.to_mail.push({mail_id:'johni@sis.in'});
			$scope.cc_mail.push({mail_id:'suhail@sis.in'},{mail_id:'rajagopalramesh1960@gmail.com'},{mail_id:'venkat@sis.in'});
