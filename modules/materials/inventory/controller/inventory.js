app.controller('inventoryCtrl', function($scope, $http, $timeout, $state) {
        ...
        $scope.onUseEdit = function(x,index,currentModalPage) {	
            console.log("view",x)
            console.log(x.id.$oid)
            $scope.detailForm.grn_no1 =  x.sisgrnno;
            $scope.detailForm.dc_no1 = x.dcno;
            $scope.detailForm.veh_no1 = x.vehicleno
            x.editype = true;
        ...
        $scope.onUseupdate = function(y,x,index,currentModalPage) {
			...
			$http({
                url: './v1/inventory_detailupdate',
                method: "POST",
                data: { 'id': x, grn_no:$scope.detailForm.grn_no1, dc_no:$scope.detailForm.dc_no1, veh_no:$scope.detailForm.veh_no1 }
            })
            .then(function (response){
                ...
                $scope.typeUse = " ";         
				  if(response.data.code == 'Success'){
                    ...
                    x = $scope.detailForm
                    x.editype = false;
                    $scope.onView($scope.detailForm,'withoutmodal')
                        }			
