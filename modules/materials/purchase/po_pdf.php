...
if($cursor)
{
    foreach($cursor as $rowData)
    {
        ...
        $taxtype = isset($rowData['vendor_details_data'][0]['taxtype']) ? $rowData['vendor_details_data'][0]['taxtype']: "GST";
        ...
        if($rowData['type']=='Standard' || $rowData['type']=='Work')
        {
            ...
            <thead>
                <tr>
                    ...
                    if($taxtype == 'IGST' )
                    {
                        $html.= "<th style='border:solid 1px #000; width:60px;' align='center'>IGST %</th>
                        <th style='border:solid 1px #000; width:100px;' align='center'>IGST &#8377;</th>";
                    }
                    else if($taxtype != 'IGST')
                    {
                        $html.= "
                        <th style='border:solid 1px #000; width:60px;' align='center'>GST %</th>
                        <th style='border:solid 1px #000; width:60px;' align='center'> <span></span>SGST  &#8377</th>
            ...
            for($i=0; $i<$productCount; $i++)
            {
                ...
                <tr>
                    ...
                    if($taxtype != 'IGST' )
                    {
                        $html.=  "<td style='border:solid 1px #000;' align='center'>".moneyFormat($sgstAmount)."</td>
                        <td style='border:solid 1px #000;' align='center'>".moneyFormat($cgstAmount)."</td>";
                    }
                    else if($taxtype == 'IGST')
                    {
                        $html.= "<td style='border:solid 1px #000;' align='center'>".moneyFormat($igst)."</td>";
                    }
                    $html.= "<td style='border:solid 1px #000;' align='right'>".moneyFormat($total)."&nbsp;</td>
                </tr>";
                $n++;
            }
            if($taxtype != 'IGST' )
        ...
        if($rowData['type']=='Work') {
           ...
           else if($rowData['trans_amount'] != '0') {
                $transGstAmt = $rowData['trans_amount']*($rowData['trans_tax']/100);
                $html.= "
                <tr>
                    <td colspan='".$colspan."'></td>
