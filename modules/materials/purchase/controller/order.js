app.controller('orderCtrl', function($scope, $http, $timeout, $state, $stateParams, orderService) {
    ...
    $scope.getcategory = function() {
        ...
        //change table type
        $scope.tableData = function(x, y, category) {
            $('#preloader').show();
            $http({
                url: './v1/getorderlist',
                method: "POST",
                data: {'cancel_status': x, 'emp_id': localStorage.getItem("user_id"), 'project_short': y, 'category': category}
            })
            .then(function (response){
    ...
    //     window.location.href = 'modules/materials/purchase/po_export.php?status='+$scope.tabletype+'&project_short='+$scope.project_short+'&category='+$scope.category+'&emp_id='+localStorage.getItem("user_id");
    // }
    ...
    $scope.export = function () {
        ...
        if (categoryParam === undefined || categoryParam === null) {
            // If category is undefined or null, use a different URL
            ...
            // If category is defined, use the original URL
